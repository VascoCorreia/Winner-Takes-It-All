...
void AprojectPlayerController::SetupInputComponent()
{
	...
	InputComponent->BindAction("SnapCameraToPlayer", IE_Pressed, this, &AprojectPlayerController::SnapCameraToPlayer);
	
	InputComponent->BindAction("ReducePlayerHealthDebug", IE_Pressed, this, &AprojectPlayerController::ReduceHealthBy5_Debug);
...
void AprojectPlayerController::ClickedActor()
{
	...
	if (hitResult.GetActor()->ActorHasTag("Player"))
	{
		...
		attackMenu = CreateWidget<UUserWidget>(this, *attackMenuToSpawn, "attackUI");
...
void AprojectPlayerController::SnapCameraToPlayer()
{
	GetCharacter()->FindComponentByClass<UCameraManager>()->CameraSnapToPlayer();
}

void AprojectPlayerController::ReduceHealthBy5_Debug()
{
	Cast<AprojectCharacter>(GetCharacter())->currentHealth -= 5.0f;
}

void AprojectPlayerController::RequestDamage_Implementation(AprojectCharacter* playerAttacking, AprojectCharacter* playerBeingAttacked)
{
	ADamageManager* damageManager = Cast<ADamageManager>(UGameplayStatics::GetActorOfClass(GetWorld(), ADamageManager::StaticClass()));

	if (damageManager)
	{
		damageManager->addAttackToQueue(playerAttacking, playerBeingAttacked);
	}
}
