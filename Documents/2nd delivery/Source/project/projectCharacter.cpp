...
void AprojectCharacter::Tick(float DeltaSeconds)
{
	...
	//make stamina that is stored in player state go down based on time
}
...
void AprojectCharacter::AddItemToInventory(FInventoryItem item)
{
...
void AprojectCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AprojectCharacter, Inventory);
}
