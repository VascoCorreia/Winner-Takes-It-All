...
void ABaseLoot::NotifyActorBeginOverlap(AActor* OtherActor)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("collision with something"));

	// Only the Server can run this code
	if (HasAuthority())
	{
		// Check if OtherActor is a AprojectCharacter
		AprojectCharacter* projectCharacter = dynamic_cast<AprojectCharacter*>(OtherActor);

		// if interacts with the character, collect loot
		if (projectCharacter != nullptr)
		{
			UE_LOG(LogTemp, Warning, TEXT("Loot collected!"));

			FInventoryItem item(Amount, Type, Image);
